...
static char *argv[MAX_ARGUMENT];
static int argc;

static struct semaphore temporary;
static thread_func start_process NO_RETURN;
static bool load (const char *cmdline, void (**eip) (void), void **esp);
static bool tokenize(char *path);
...
void push_args_to_stack(void** esp) {
    ...
    for (int i = argc - 1; i >= 0; i--) {
        ...
        *esp -= len;
        ...
        argv[i] = *esp; // save new address in argv
    }
...
bool tokenize(char *path) {
    char *rest = 
           calloc(strlen(path), sizeof(char));
    memcpy(rest, path, strlen(path));

    char *token;

    while ((token = strtok_r(rest, " ", &rest))) {
        if (argc >= MAX_ARGUMENT || strlen(token) > MAX_ARGUMENT_LENGTH)
            return false;
        argv[argc] = token;
        argc += 1;
    }
